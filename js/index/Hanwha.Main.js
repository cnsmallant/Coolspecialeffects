...
require(["contents/Plugin",
    ...
    function pageChangeMotion(isUp) {
        ...
        if (nextCount == 1 || nextCount == 3) TweenMax.to($("#next_scroll_info"), 1, { alpha: 0 });
    ...
    function onDirectPageChangeMotion(idx) {
        ...
        for (var i = 0; i < gap; ++i) {
            var plus = 0;
            if (!isUp && setPrevCount == 1 && i != 0)

                plus = 1;

            TweenMax.to(obj, 0, {
                data: 0, delay: i * 0, onComplete: function (idx, isUp, gap) {
                    if (isUp) {
